...
public class ServerMsgUtil {
	 ...
	 * @throws IOException 
	 */
	public static byte[] creatMsg(MsgHead head) throws IOException {
		...
		if (type == IMsgType.MSG_LOGINRESP) {
			Msg_LoginResp msgLoginResp = (Msg_LoginResp) head;
			dous.writeByte(msgLoginResp.getState());
		...
		DataInputStream dis = new DataInputStream(bais);
			try {
				...
				if (type == IMsgType.MSG_LOGIN) {
					Msg_Login msgLogin = new Msg_Login();
					copyMsgHead(head, msgLogin);
					byte[] bs = new byte[20];
					dis.read(bs);
					msgLogin.setUsername(new String(bs).trim());
					dis.read(bs);
					msgLogin.setPassword(new String(bs).trim());
					return msgLogin;
				}else if(type == IMsgType.MSG_TALK){
